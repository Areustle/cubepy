...
def test_split_1():
    ...
    c, h, v = region.split(c, h, v, s)

    assert np.all(c[0] == [[0.25], [0.75]])
    assert np.all(h[0] == [[0.25], [0.25]])
    assert np.all(v == [0.5])
...
def test_split_2():
    ...
    assert np.all(c[0] == np.array([[[0.5], [0.5 * np.pi]]]))
    assert np.all(h[0] == np.array([[[0.5], [0.5 * np.pi]]]))
    assert np.all(v == [1, np.pi])

    # s = np.array([1])
    # c1, h1, v1 = region.split(c, h, v, s)
    # assert np.all(c1 == [[[0.5], [0.5]], [[0.25 * np.pi], [0.75 * np.pi]]])
    # assert np.all(h1 == [[[0.5], [0.5]], [[0.25 * np.pi], [0.25 * np.pi]]])
    # assert np.all(v1 == [0.5 * np.pi])
    #
    # s = np.array([0])
    # umask = np.array([True])
